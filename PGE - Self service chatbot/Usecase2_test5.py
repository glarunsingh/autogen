...
def analyze_and_start_autogen_qa(content):
    ...
    if data_collection_messages:
        final_data["data_collection_response"] = data_collection_messages[-1]["content"]  # Last meaningful messagea
...
def process_document_or_summary(doc_path=None):
    ...
    output_filename = "final_collected_data.json"

    # Get the current directory of the script
    output_filepath = os.path.join(os.path.dirname(__file__), output_filename)

    # Write the final collected data to a JSON file
    with open(output_filepath, 'w') as json_file:
        json.dump(final_data, json_file, indent=2)

    print(f"\nFinal Collected Data has been saved to: {output_filepath}")
    return final_data
    return final_data
